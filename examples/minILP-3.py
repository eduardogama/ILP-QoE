...
for i in nodes:
	if nodes[i][0] == "ap\n":
		...
		path[i] = g.dijkstra(graph,i,dst,dist,cparent)
...
for i in clientPaths:
	for r in clientPaths[i]:
		...
		if (i,j1) not in costs:
			costs[(i,j1)] = 0
		costs[(i,j1)] += users[i] * (1-nodes[j][1]/n_users)
...
print("edgePeer", edgePeer) # [0, 1, 2, 3, 4, 5, 6, 7]
print("costs", costs) # {(3, 7): 4, (3, 0): 4, (3, 1): 4, (3, 3): 4, (4, 7): 4, (4, 0): 4, (4, 1): 4, (4, 4): 4, (5, 7): 3, (5, 0): 3, (5, 2): 3, (5, 5): 3, (6, 7): 4, (6, 0): 4, (6, 2): 4, (6, 6): 4}
print("clientPaths", clientPaths) # {3: {4: (7, 0), 3: (0, 1), 2: (1, 3), 1: (3, 3)}, 4: {4: (7, 0), 3: (0, 1), 2: (1, 4), 1: (4, 4)}, 5: {4: (7, 0), 3: (0, 2), 2: (2, 5), 1: (5, 5)}, 6: {4: (7, 0), 3: (0, 2), 2: (2, 6), 1: (6, 6)}}
print("fixedUsersPerNode",fixedUsersPerNode) # {0: 15, 1: 8, 2: 7, 3: 4, 4: 4, 5: 3, 6: 4, 7: 15}
print(costsPerLink) # {(7, 0): 15, (0, 1): 8, (0, 2): 7, (1, 3): 4, (1, 4): 4, (2, 5): 3, (2, 6): 4}


l_r = {} # {4: [(7, 0)], 3: [(0, 1), (0, 2)], 2: [(1, 3), (1, 4), (2, 5), (2, 6)], 1: [(3, 3), (4, 4), (5, 5), (6, 6)]}
node_level = {} # {7: 4, 0: 3, 1: 2, 3: 1, 4: 1, 2: 2, 5: 1, 6: 1}
...
for i in clientPaths:
	for l_i_r in clientPaths[i]:
		...
		if clientPaths[i][l_i_r] not in l_r[l_i_r]:
			...
			node_level[clientPaths[i][l_i_r][0]] = l_i_r
...
def subtourelim(model, where):
	if where == GRB.Callback.MIPSOL:
		...
		users_lvl =  users_lvl_copy = {i : node_level[j] for i,j in selected }
				
		print("====")
		print("users_lvl",users_lvl)
		print("l_r", l_r)
		for r in l_r:
			for j1,j2 in l_r[r]:
				sum_ = [users[i] for i in users if (users_lvl[i] == r) and (clientPaths[i][r] == (j1,j2))]

				print(j1,j2,"->",sum(sum_) * maxBitrate <= flinks[j1,j2])
				sys.stdin.read(1)

				model.cbLazy(quicksum(users[i] * maxBitrate * vals[i, users_lvl[i]] for i in users if r in clientPaths[i]) <= links[j1,j2] for r in l_r)
		
			for i in users:
				users_lvl[i] = users_lvl[i] - 1
